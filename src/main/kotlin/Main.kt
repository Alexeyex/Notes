fun main() {
    NoteService.addNote(Note(title = "Добрый день", text = "Good day"))
    NoteService.addNote(Note(title = "Добрый вечер", text = "Good evening"))
    ...
    NoteService.editNote(
        NoteService.getNoteById(1), "Заголовок 1 отредактирован",
        "Первая запись исправлена"
    )
    NoteService.editNote(
        NoteService.getNoteById(2), "Заголовок 2 отредактирован",
        "2я запись исправлена"
    )

    NoteService.addComment(
        Comment(text = "комментарий к комментарию"),
        NoteService.getNoteById(2).getCommentById(1)
    )
    NoteService.addComment(
        Comment(text = "еще один комментарий к комментарию"),
        NoteService.getNoteById(2).getCommentById(1)
    )
    NoteService.addComment(
        Comment(text = "и еще один комментарий к комментарию"),
        NoteService.getNoteById(2).getCommentById(1)
    )

    NoteService.addComment(
        Comment(text = "комментарий к комментарию к комментарию"),
